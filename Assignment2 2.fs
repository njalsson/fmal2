...
let rec allPatternHelper(n: expr)=
    match n with
    | Pair(Num x,Num y)-> true
    | Pair(Var x, Var Y) -> true
    | Pair(Num x, Var y) -> false
    |Pair(x,y) -> allPatternHelper(Pair(y,x))
    |_ -> false

let rec checkAllPatterns (e : expr)= 
    match e with
    | Let(x,y,z) -> checkPattern(x) && checkAllPatterns(y) && checkAllPatterns(z)
    | Pair(x,y) -> allPatternHelper(Pair(x,y))
    | _ -> true
checkAllPatterns (Pair (Num 1, Let (PPair (PVar "x", PVar "x"), Num 2, Num 3)));;
allPatternHelper(Pair(Num 1, Var "1"))
allPatternHelper(Pair (Var "y", Num 9))
allPatternHelper(Pair(Var "x", Var "y"))

checkAllPatterns(Pair (Var "y", Num 9))
